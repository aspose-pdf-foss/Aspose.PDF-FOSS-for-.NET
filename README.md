...
```xml
<PackageReference Include="Aspose.PDF.FOSS" Version="26.7.0" />
```
...
- Dynamic XFA forms are read and can be flattened to real AcroForm pages, and XFA datasets
  round-trip through the AcroForm `/XFA` entry and sync with AcroForm fields, but fine-grained
  authoring of individual XFA dataset fields is still limited.
- `PDF3DAnnotation` content (artwork, views, cross-sections, lighting) is read and round-trips,
  ...
  3D model.
- The low-level `Aspose.Pdf.Comparison.Diff` edit-operation model is available, but a
  higher-level "compare two PDFs and produce a visual diff document" workflow is not.
