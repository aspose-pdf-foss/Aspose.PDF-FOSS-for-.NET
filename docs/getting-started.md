...
The only runtime dependency is the **`System.Drawing.Common`** NuGet package, used by a handful of image-interop members that are Windows-only on .NET 8 (they throw `PlatformNotSupportedException` on Linux/macOS). The rest of the library — text, forms, parsing, encryption & signing, and page→image rendering via `PngDevice`/`JpegDevice` — is pure-managed and runs on Windows, macOS, and Linux.
...
> version; documents opened from a file keep their own version.
...
// From Markdown
using var doc5 = Document.Open("readme.md", new Aspose.Pdf.Converters.MdLoadOptions());

// From SVG
using var doc6 = Document.Open("drawing.svg", new Aspose.Pdf.Converters.SvgLoadOptions());
```

For PDF and HTML, both the constructor (`new Document(...)`) and the static
factory (`Document.Open(...)`) perform the load/convert. For **Markdown and
SVG**, only `Document.Open(path, MdLoadOptions)` / `Document.Open(path,
SvgLoadOptions)` run the converter — there is no equivalent constructor overload
that consumes those options, so use `Document.Open(...)` for Markdown and SVG.
...
`doc.Save()` (no argument) writes back to the source. It works when the document was opened from a file path or a writable file stream; opening from a byte array or a read-only stream and calling `doc.Save()` throws `InvalidOperationException` — use `doc.Save(stream)` / `doc.Save(path)` instead.
