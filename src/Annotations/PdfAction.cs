...
public class PdfAction : IAppointment
{
    ...
    public Annotations.ActionCollection Next =>
        // LIVE: bound to this action's dictionary, so Add/Delete rewrite /Next in
        // the document rather than mutating a detached snapshot.
        _reader is null
            ? new Annotations.ActionCollection()
            : Annotations.ActionCollection.ForNextChain(_dict, _reader);
    ...
    internal static PdfAction Create(PdfDictionary dict, PdfReader reader)
    {
        ...
        return s switch
        {
            ...
            "Hide" => new HideAction(dict, reader),
...
public sealed class GoToAction : PdfAction
{
    ...
    /// <summary>
    /// Create a GoTo action targeting the given explicit destination (API compatibility).
    /// </summary>
    ...
    /// <summary>
    /// Create a GoTo action targeting the first view of the given page (API compatibility).
    /// </summary>
    ...
    /// <summary>
    /// Create a GoTo action targeting the page at the given 1-based page number (API compatibility).
    /// </summary>
    ...
    /// <summary>Create an empty GoTo action (API compatibility).</summary>
    public GoToAction()
        : base(EmptyGoToDict())
    {
    }
...
public sealed class NamedAction : PdfAction
{
    ...
    // it is serialised as a JavaScript action running this exact
    // script (every other PredefinedAction stays /S/Named/N/<name>,
    // e.g. Print, NextPage, Edit_Find; encryption plays no part).
    private const string PrintDialogScript = "this.print(true);\r";
    ...
    public NamedAction(Annotations.PredefinedAction action) : base(BuildDict(action))
    {
    }

    private static PdfDictionary BuildDict(Annotations.PredefinedAction action)
    {
        var dict = new PdfDictionary();
        ...
        dict.Set("S", new PdfName("Named"));
        dict.Set("N", new PdfName(action.ToString()));
        return dict;
    }
...
/// <summary>Hide action (/S /Hide, PDF 32000 §12.6.4.10): sets the hidden flag of
/// the named field(s) or annotation(s). Serialised as
/// <c>&lt;&lt;/S/Hide /T (name-or-array) /H bool&gt;&gt;</c> — /H is written for both
/// values (always written explicitly rather than left to the spec default).</summary>
public sealed class HideAction : PdfAction
{
    internal HideAction(PdfDictionary dict, PdfReader reader) : base(dict, reader) { }

    /// <summary>Create an empty Hide action (targets set later).</summary>
    public HideAction() : base(BuildDict(null, true)) { }

    /// <summary>Hide the field the given annotation belongs to.</summary>
    public HideAction(Annotation annotation) : this(annotation, true) { }

    /// <summary>Hide (or show) the field the given annotation belongs to.</summary>
    public HideAction(Annotation annotation, bool isHidden)
        : base(BuildDict(TargetName(annotation), isHidden)) { }

    /// <summary>Hide the field with the given fully qualified name.</summary>
    public HideAction(string fieldName) : this(fieldName, true) { }

    /// <summary>Hide (or show) the field with the given fully qualified name.</summary>
    public HideAction(string fieldName, bool isHidden)
        : base(BuildDict(new PdfString(System.Text.Encoding.UTF8.GetBytes(fieldName ?? string.Empty)), isHidden)) { }

    /// <summary>Hide the fields the given annotations belong to.</summary>
    public HideAction(Annotation[] annotations) : this(annotations, true) { }

    /// <summary>Hide (or show) the fields the given annotations belong to.</summary>
    public HideAction(Annotation[] annotations, bool isHidden)
        : base(BuildDict(TargetArray(annotations), isHidden)) { }

    /// <summary>Hide the fields with the given fully qualified names.</summary>
    public HideAction(string[] names) : this(names, true) { }

    /// <summary>Hide (or show) the fields with the given fully qualified names.</summary>
    public HideAction(string[] names, bool isHidden)
        : base(BuildDict(NameArray(names), isHidden)) { }

    /// <summary>Whether the action hides (true) or shows (false) its targets (/H).</summary>
    public bool IsHidden
    {
        get => Dict.Get("H") is not PdfBoolean b || b.Value; // /H defaults to true
        set => Dict.Set("H", value ? PdfBoolean.True : PdfBoolean.False);
    }

    // An annotation-targeted Hide action stores the ANNOTATION DICTIONARY itself
    // under /T (PDF 32000 §12.6.4.10 allows an annotation or a field name; the
    // corpus reads /T back as a dictionary whose /Type is Annot for this ctor
    // family). Name-targeted ctors store text strings instead.
    private static PdfObject? TargetName(Annotation annotation) => annotation?.Dict;

    private static PdfArray TargetArray(Annotation[] annotations)
    {
        var arr = new PdfArray();
        foreach (var a in annotations ?? System.Array.Empty<Annotation>())
            if (TargetName(a) is { } t) arr.Add(t);
        return arr;
    }

    private static PdfArray NameArray(string[] names)
    {
        var arr = new PdfArray();
        foreach (var n in names ?? System.Array.Empty<string>())
            arr.Add(new PdfString(System.Text.Encoding.UTF8.GetBytes(n ?? string.Empty)));
        return arr;
    }

    private static PdfDictionary BuildDict(PdfObject? target, bool isHidden)
    {
        var dict = new PdfDictionary();
        dict.Set("S", new PdfName("Hide"));
        if (target is not null) dict.Set("T", target);
        dict.Set("H", isHidden ? PdfBoolean.True : PdfBoolean.False);
        return dict;
    }
}
...
public sealed class JavascriptAction : PdfAction
{
    ...
    /// <summary>Create a JavaScript action with the given script source (API compatibility).</summary>
