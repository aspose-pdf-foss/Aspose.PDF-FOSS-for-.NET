...
// SIDE-BY-SIDE layout holds for the first page only (the reference resumes the
// content full-width at the wrapper's left padding after a page break), the
// wrapper's 220px bottom padding, and the date/footer tail. The reference
// paces content at the UA 18px line on the sheet's 16px font with a uniform
...
internal static partial class HtmlToPdfConverter
{
    ...
    // Block gap measured on the reference: the white band between any two content
    ...
    // base (measured: the reference's first content line box opens 12 pt
        ...
        double pageWidth, double pageHeight, string? basePath = null)
    {
        ...
        // resolver, whose parent-directory fallback recovers the reference's own
        // recovery (a base pointing INTO the img folder doubles the folder on
        // the naive join; the parent-dir retry finds the file).
        byte[]? logoBytes = null;
        ...
        var bandColor = css.TryGetValue(".article-pdf__title", out var titleRule)
            ...
            && ParseCssColor(bandV) is { } bv ? bv : Color.FromRgb(226, 23, 31);
        ...
        // padded box (measured on the reference; the in-flow variant
        ...
        var descColor = css.TryGetValue(".article-pdf__description", out var descRule)
            ...
            && ParseCssColor(descColV) is { } dcv ? dcv : Color.FromRgb(23, 54, 93);
