...
// symbol faces the reference uses. Every constant below is either read from the
// stylesheet or measured on the reference render.
internal static partial class HtmlToPdfConverter
{
    ...
    private sealed class BsButton
    {
        ...
        public Color Fill = Color.FromRgb(236, 236, 236);
        public Color Border = Color.FromRgb(145, 150, 156);
        public Color Fg = Color.FromRgb(51, 51, 50);
    }

    // The reference's content box: 90 pt side margins, 72 pt top/bottom (the body
    // background fill spans exactly (90,72)-(pageW-90,pageH-72) on the reference).
    ...
    // box — 2 pt to the sides, 1.5 pt above/below (measured on the reference).
    private const double BsButtonOutlineX = 2.0;
    private const double BsButtonOutlineY = 1.5;

    // The MVC-template (navbar + jumbotron) arm — all measured on the reference:
        ...
        double pageWidth, double pageHeight)
    {
        ...
        // default the reference substitutes (Arial).
        var face = "Arial";
        ...
        var bodyColor = body.TryGetValue("color", out var bodyColV)
            && ParseCssColor(bodyColV) is { } bc ? bc : Color.FromRgb(0, 0, 0);
        ...
        var borderCol = Color.FromRgb(195, 198, 201);
        var tableBg = Color.FromRgb(255, 255, 255);
        ...
        (Color fill, Color border, Color fg) BtnColors(string classes)
        {
            foreach (var cls in classes.Split(' ', StringSplitOptions.RemoveEmptyEntries))
                    ...
                    && css.TryGetValue("." + cls, out var vr))
                {
                    var fill = vr.TryGetValue("background-color", out var bgv)
                        && ParseCssColor(bgv) is { } f ? f : Color.FromRgb(236, 236, 236);
            ...
            return (Color.FromRgb(236, 236, 236), Color.FromRgb(145, 150, 156), bodyColor);
        }
        ...
        Color jumboBtnFill = Color.FromRgb(0x33, 0x7a, 0xb7);
        Color jumboBtnBorder = Color.FromRgb(0x2e, 0x6d, 0xa4);
        if (jumboDoc)
        {
        ...
        // A glyphicon: the system symbol face the reference substitutes, embedded
        ...
        if (jumboDoc)
        {
            ...
            if (jumboBtnLabel.Length > 0)
            {
                ...
                EmitRun("FB", btnLgFs, jX + JmbPadXPt + 0.75 + btnLgPadX,
                    ...
                    jumboBtnLabel, Color.FromRgb(255, 255, 255));
            ...
            if (jumboBtn2Label.Length > 0)
            {
                ...
                Box(jX + JmbPadXPt - BsButtonOutlineX, pen - BsButtonOutlineY,
                    ...
                    null, Color.FromRgb(0, 0, 0), 1.0);
                Box(jX + JmbPadXPt, pen, bw, JmbBtnHPt,
                    Color.FromRgb(255, 255, 255), Color.FromRgb(0xcc, 0xcc, 0xcc), 0.75);
