...
internal static partial class HtmlToPdfConverter
{
    ...
    //
    // All geometry from the reference: content x = 96 + padding; the first section
            ...
            RegexOptions.IgnoreCase))
        {
                ...
                RegexOptions.IgnoreCase))
            {
                ...
                var bCol = Regex.Match(st, @"border-color:\s*(#[0-9a-fA-F]{3,6})") is { Success: true } bcm
                    ? ParseCssColor(bcm.Groups[1].Value) ?? Color.FromRgb(0, 0, 0) : Color.FromRgb(0, 0, 0);
                    ...
                    RegexOptions.IgnoreCase))
                {
                    ...
                    HStroke(rx, rx + uw, uy + uh + 0.38, Color.FromRgb(0, 0, 0), 0.75);
                    uy += uh + 0.7;
                }
                ...
                foreach (Match trM in Regex.Matches(resp, @"<tr>\s*([\s\S]*?)</tr>", RegexOptions.IgnoreCase))
                {
                    ...
                    for (var ci = 0; ci < cells.Count; ci++)
                    {
                        ...
                        {
                            Box(cx + pad, rowY + pad, 9.0, 9.0, Color.FromRgb(0, 0, 0), 0.75);
                            cx += pad + 9.75 + pad;
                        }
