...
internal static partial class HtmlToPdfConverter
{
    ...
    internal static (double Min, double Max) LayoutCellSpan(Cell cell, double pad, double border)
    {
        ...
        foreach (var cp in cell.Paragraphs)
        {
            ...
            double W(string t)
            {
                ...
                try
                {
                    return Aspose.Pdf.Text.FontRepository.FindFont(cface)?.MeasureString(t, cfs)
                           ?? t.Length * cfs * 0.5;
                }
    ...
    private static int WrappedLineCount(string text, string face, double size, double width, bool noWrap)
    {
        ...
        double W(string t)
        {
            ...
            try
            {
                return Aspose.Pdf.Text.FontRepository.FindFont(face)?.MeasureString(t, size)
                       ?? t.Length * size * 0.5;
            }
    ...
    /// <summary>The base type an in-page HtmlFragment sets in: the size and family its own
    /// <c>body { … }</c> rule declares. A fragment with no assigned TextState inherits this
    /// instead of the 11 pt Standard-14 default, exactly as a browser would. The family is
    /// returned only when the first named face of the stack is INSTALLED, so callers can
    /// measure it; size is 0 and face null when the document declares neither.</summary>
    internal static (double SizePt, string? Face) BodyCssFont(string html)
    {
        if (string.IsNullOrEmpty(html)) return (0, null);
        var css = ParseStyleSheet(html);
        if (!css.TryGetValue("body", out var body)) return (0, null);
        var sizePt = 0.0;
        if (body.TryGetValue("font-size", out var fs) && TryParseLength(fs, out var fsPt) && fsPt > 0)
            sizePt = fsPt;
        string? face = null;
        if (body.TryGetValue("font-family", out var ff))
