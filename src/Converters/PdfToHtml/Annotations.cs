...
public sealed partial class PdfToHtmlConverter
{
            ...
            double pageW = 0, double pageH = 0)
    {
        ...
        // A mask group with an effectively unbounded BBox (generators write
        // [-32768..32767] around a gradient) rasterises only where it can matter:
        // the page box. Without the clamp the raster blows the size guard and the
        // mask is silently dropped.
        if (pageW > 0 && pageH > 0)
        {
            bx0 = Math.Max(bx0, 0); by0 = Math.Max(by0, 0);
            bx1 = Math.Min(bx1, pageW); by1 = Math.Min(by1, pageH);
            if (bx1 - bx0 < 0.01 || by1 - by0 < 0.01) return null;
        }
    ...
    private static void EmitSvgShading(StringBuilder svgPaths, Aspose.Pdf.Shading.ShadingBase? shading,
        string? clipD, ContentRenderState ct, int seq, double pageHeight)
    {
        ...
        switch (shading)
        {
            ...
            {
                var (x1, y1) = Dp(ct, ax.X0, ax.Y0);
                var (x2, y2) = Dp(ct, ax.X1, ax.Y1);
        ...
        if (string.IsNullOrEmpty(region))
        {
            ...
            var (bx0, by0) = Dp(ct, box[0], box[1]);
            var (bx1, by1) = Dp(ct, box[2], box[3]);
    ...
    /// space — the text content consumers expect.
    /// </summary>
