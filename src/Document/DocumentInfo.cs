...
public sealed class DocumentInfo
{
    ...
    public string? Producer
    {
        ...
        set
        {
            ...
            SetString("Producer", value);
        }
    }

    /// <summary>The caller assigned Producer through the public setter, so the
    /// save-time library stamp must not overwrite it (a value merely carried by
    /// the loaded file does not set this).</summary>
    internal bool ProducerAssigned;

    /// <summary>Save-time stamp write: bypasses <see cref="ProducerAssigned"/>.</summary>
    internal void StampProducer(string value) => SetString("Producer", value);
    ...
    private void SetString(string key, string? value)
    {
        ...
        if (value is null)
            ...
            // Text outside Latin1 (e.g. Œ, CJK) upgrades to UTF-16BE with a BOM;
            // a bare Latin1 write flattened such characters to their fallback
            // (Œ → O) and the /Info string no longer round-tripped.
            _dict.Set(key, Forms.Field.EncodePdfTextString(value));
        FlushDirty();
    }
