...
public sealed partial class Document : IDisposable
{
        ...
        ColumnInfo info, double marginLeft, double contentWidth)
    {
        ...
        // A column never reaches past the page's right content edge: declared
        // widths that would (two 250 pt columns plus a 12 pt gap in a 468 pt
        // box) are cut there, and the text in that column wraps to what is left.
        var rightEdge = marginLeft + contentWidth;
        for (var i = 0; i < count; i++)
            if (lefts[i] + widths[i] > rightEdge && rightEdge - lefts[i] > 1)
                widths[i] = rightEdge - lefts[i];
        return (lefts, widths);
    }
    ...
    /// emphasis from the <c>&lt;b&gt;</c>/<c>&lt;strong&gt;</c>, <c>&lt;i&gt;</c> and
    /// <c>&lt;u&gt;</c> ranges
    /// the parser recorded, each carrying the <c>&lt;a href&gt;</c> it sits inside so the
    /// writer can put a Link annotation over it. Returns null when the block needs no
    /// split — one style throughout, nothing underlined and no anchor — so it keeps
    /// drawing through the ordinary whole-block writer.</summary>
    private static System.Collections.Generic.List<(int Start, int Length, bool Bold,
        bool Italic, bool Underline, Hyperlink? Link)>?
        HtmlEmphasisRuns(Converters.HtmlToPdfConverter.Block b)
    {
        var n = b.Text?.Length ?? 0;
        if (n == 0) return null;
        var haveBold = b.BoldRuns is { Count: > 0 };
        var haveItalic = b.ItalicRuns is { Count: > 0 };
        var haveUnder = b.UnderlineRuns is { Count: > 0 };
        if (!haveBold && !haveItalic && !haveUnder) return null;
        ...
        Mark(b.ItalicRuns, italic);
        Mark(b.UnderlineRuns, under);
        // One Hyperlink instance per anchor, shared by every character it covers:
        // the writer coalesces neighbouring cells by REFERENCE, so one anchor split
        // across bold/underline runs still emits a single annotation per line.
        var links = new Hyperlink?[n];
        if (b.Anchors is { Count: > 0 })
            foreach (var (aStart, aLen, aUrl) in b.Anchors)
            {
                if (aLen <= 0 || string.IsNullOrEmpty(aUrl)) continue;
                var link = new WebHyperlink(aUrl);
                for (var i = Math.Max(aStart, 0); i < Math.Min(aStart + aLen, n); i++)
                    links[i] = link;
            }

        var runs = new System.Collections.Generic.List<(int Start, int Length, bool Bold,
            bool Italic, bool Underline, Hyperlink? Link)>();
        var start = 0;
        for (var i = 1; i <= n; i++)
            if (i == n || bold[i] != bold[start] || italic[i] != italic[start]
                || under[i] != under[start]
                || !ReferenceEquals(links[i], links[start]))
            {
                runs.Add((start, i - start, bold[start], italic[start], under[start],
                    links[start]));
                start = i;
            }
        // Uniformly-styled, undecorated and unlinked: nothing the block-wide face
        // cannot express.
        if (runs.Count == 1 && !runs[0].Italic && !runs[0].Underline && runs[0].Link is null)
            return null;
        return runs;
    }
    ...
    {
        try { return Text.FontRepository.TryFindFont("Times New Roman", ignoreCase: true); }
        catch { return null; }
    }
    ...
    /// <summary>Re-raster one JPEG frame as PNG with the built-in managed decoder,
    /// which reads baseline and progressive alike. Null when the bytes are not a JPEG
    /// this decoder can read, so the caller degrades to "no frames" exactly as an
    /// unavailable codec would.</summary>
    private static byte[]? JpegFrameAsPng(byte[] data)
    {
        if (data.Length < 4 || data[0] != 0xFF || data[1] != 0xD8) return null;
        try
        {
            var (pixels, w, h, components) = IO.Filters.JpegDecoder.Decode(data);
            if (w <= 0 || h <= 0 || pixels.Length == 0) return null;
            // The decoder reports one component for greyscale and three for anything it
            // colour-converted (YCbCr, CMYK and YCCK all arrive here as RGB).
            if (components == 1 && pixels.Length >= w * h)
                return IO.PngEncoder.Encode(pixels, w, h, colorType: 0);
            if (components >= 3 && pixels.Length >= (long)w * h * 3)
                return IO.PngEncoder.Encode(pixels, w, h, colorType: 2);
            return null;
        }
        catch { return null; }
    }
    ...
    private static System.Collections.Generic.List<byte[]>? TryDecodeImageFramesAsPng(byte[] data)
    {
        ...
        // no codec, and leaving it to the Windows-only one below dropped the picture
        // everywhere else.
        if (IO.BmpDecoder.IsBmp(data) && IO.BmpDecoder.DecodeAsPng(data) is { } bmpPng)
            return new System.Collections.Generic.List<byte[]> { bmpPng };
        // Below is the System.Drawing codec fallback, which exists only on Windows. Off
        // Windows the managed decoders are the whole story, so JPEG joins them here: a
        // BASELINE JPEG never reaches this method (it embeds verbatim), but a PROGRESSIVE
        // one does, because the PDF image decoder is baseline-only and the frame has to be
        // re-rasterised first. That re-raster was the Windows-only codec below, so every
...
#pragma warning restore CA1416
    ...
    internal static bool TryGetImageNaturalSizePt(byte[] d, bool applyResolution, out double widthPt, out double heightPt)
    {
        ...
        // optional 'res' box, which we ignore for consistency with unsized generator images).
