...
public sealed partial class Document
{
        ...
        double fillScale, double strokeScale, bool recolor = false)
    {
        ...
        var gsInfo = new Dictionary<string, (double? Ca, double? CA, string? Bm, bool? Sm)>(StringComparer.Ordinal);
        ...
        var stack = new Stack<(SimMatrix Ctm, double FillA, double StrokeA, string Bm, bool Sm,
        ...
        while (pos < text.Length)
        {
...

            {
                ...
                switch (token)
                {
                    ...
                    case "q":
                        stack.Push((ctm, fillA, strokeA, blend, softMask, fillCol, strokeCol));
                        break;
                    case "Q":
                        if (stack.Count > 0) (ctm, fillA, strokeA, blend, softMask, fillCol, strokeCol) = stack.Pop();
                        break;
                    ...
                    case "gs" when lastName is not null:
                        if (gsInfo.TryGetValue(lastName, out var info))
                        {
                            ...
                            if (info.Sm is { } smV) softMask = smV;
                        }
                        break;
                    case "re":
                        if (nums.Count >= 4)
