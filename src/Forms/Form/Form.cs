...
public sealed partial class Form : ICollection<Aspose.Pdf.Annotations.WidgetAnnotation>
{
    ...
    private void PlaceFieldWidgets(PdfDictionary fieldDict, PdfReader reader, int pageIndex)
    {
        ...
        foreach (var widget in CollectWidgetDicts(fieldDict, reader))
        {
            ...
            // An /Annots entry is an annotation: a merged field dict placed as a
            // widget must carry the annotation identity, or a reloaded document
            // surfaces it as a bare Annotation instead of a WidgetAnnotation
            // (/Type/Annot /Subtype/Widget is written on merged fields).
            // Only a dict that IS a visual widget (has /Rect) is stamped — a
            // group root placed through the no-widget fallback (a radio group
            // whose option rects arrive later from layout) must stay bare.
            if (widget.GetName("Subtype") is null && widget.ContainsKey("Rect"))
            {
            ...
            // A JSON-imported widget whose source carried no /P stays without one
            // (the _NoPageRef hint from ImportFieldEntry).
            if (widget.Get("_NoPageRef") is not null)
                widget.Remove("_NoPageRef");
            else
    ...
    internal static void EmbedDefaultAppearanceFont(Field field)
    {
        ...
        if (da?.EmbeddedFont is not { } font) return;
        var ttf = font.SourceFontData?.TtfData;
        if (ttf is null || ttf.Length == 0) return;
        // A caller who OPENED a face and handed it to the field has no other way to get
        // those glyphs into the document, so the face travels with the form whether or
        // not IsEmbedded was ever touched — only an explicit IsEmbedded = false keeps it
        // out. (Measured: DefaultAppearance(OpenFont(...), ...) puts
        // the program into /DR as a Type0 face and repoints /DA at it.)
        if (font.EmbeddingRefusedExplicitly) return;
