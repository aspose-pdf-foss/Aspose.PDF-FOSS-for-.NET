...
/// <summary>The SASLprep (RFC 4013) profile over the stringprep framework (RFC 3454),
/// used to prepare a password for AES-256 key derivation so that visually equivalent
/// passwords derive the same key.</summary>
internal static class SaslPrepProfile
{
    /// <summary>Prepare a password for AES-256 (R6) key derivation. Applies SASLprep,
    /// falling back to the original string when SASLprep rejects it, which is the
    /// lenient behaviour PDF processors use so a non-conforming password still
    /// round-trips.</summary>
    internal static string PrepareForKeyDerivation(string password)
    {
        ...
        catch (SaslPrepRejectedException) { return password; }
    }
    ...
    internal static string SaslPrep(string input)
    {
        ...
        var normalized = Normalization.Normalize(mapped.ToString());
        ...
        foreach (var cp in CodePoints(normalized))
        {
            ...
            if (table is not null)
                throw new SaslPrepRejectedException(
...
/// <summary>Raised when a string does not satisfy the SASLprep profile.</summary>
internal sealed class SaslPrepRejectedException : PdfException
{
    internal SaslPrepRejectedException(string message) : base(message) { }
}
