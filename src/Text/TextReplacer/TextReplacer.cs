...
public sealed partial class TextReplacer
{
    ...
    /// <summary>When set, a cross-operator replacement that changes the matched span's
    /// ADVANCE re-seats the rest of the line by the difference. Set for
    /// <see cref="TextReplaceOptions.ReplaceAdjustment.AdjustSpaceWidth"/> only:
    /// <see cref="TextReplaceOptions.ReplaceAdjustment.None"/> exists precisely so the
    /// surviving text does NOT move.</summary>
    internal bool ShiftFollowersByAdvance { get; set; }

    private bool _shiftedFollowers;


    /// <summary>True when the last Replace re-seated a line's followers itself, so a
    /// caller's own follower pass must not shift the same line a second time.</summary>
    internal bool ShiftedFollowers => _shiftedFollowers;
    ...
    /// <summary>When set, a CID-fallback replacement embeds THIS family instead of
    /// walking the source-family-preserving candidate list. The explicit-ReplaceFonts
    /// font assignment re-dresses an absorbed run through the substitution scan
    /// (caller sources → the named Han face → per-user) — the source family must not
    /// re-bind (measured: a run whose caller-assigned Times cannot cover CJK
    /// re-dresses in SimSun even while the source family SimHei is installed
    /// per-user, yet an assignment-free replace of the same run family-preserves).</summary>
    internal string? ForcedCidFallbackFamily { get; set; }
